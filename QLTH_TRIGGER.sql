...
DECLARE
	...
	@I_END DATE
BEGIN
	SELECT @I_START = Inserted.NGAYBATDAU, @I_END = Inserted.NGAYKETTHUC
	FROM Inserted

	IF ( @I_START >= @I_END)
	BEGIN
		RAISERROR (N'NGÀY BẮT ĐẦU KHÔNG ĐƯỢC NHỎ HƠN NGÀY KẾT THÚC', 16, 1)
		ROLLBACK
	END
END

GO
CREATE TRIGGER FK_USER
